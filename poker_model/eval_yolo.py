...
def find_id(filename, annot):
    for j in annot["images"]:
        if filename==j["file_name"]:
            return j['id']


def generate_results(trt_yolo, imgs_dir, jpgs, results_file, non_coco, annot):
...
def main():
    ...
    with open('../share/yolo2coco.json', "r") as json_file:
        annotation_data=json_file.read()
        annot = json.loads(annotation_data)
    json_file.close()
    ...
    generate_results(trt_yolo, args.imgs_dir, jpgs, results_file,
                     non_coco=args.non_coco, annot=annot)
